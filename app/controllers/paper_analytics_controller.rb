class PaperAnalyticsController < ApplicationController
  ...
  def index
    ...
    if params[:filter] == nil
      ...
      if params[:relation] == "paper_subjects"
          @papers = Paper.includes(:paper_subject).order("paper_subjects.title  #{order}").paginate(:page => params[:page], :per_page => 10)
      elsif params[:relation] == "grades"
          @papers = Paper.includes(:grades).order("grades.name  #{order}").paginate(:page => params[:page], :per_page => 10)
      elsif params[:relation] == "questions"
          @papers = Paper.left_joins(:questions).group(:id).order("COUNT(questions.id) #{order}").paginate(:page => params[:page], :per_page => 10)
      elsif params[:relation] == "student_open_paper_logs" 
          @papers = Paper.left_joins(:student_open_paper_logs).group(:id).order("COUNT(student_open_paper_logs.id) #{order}").paginate(:page => params[:page], :per_page => 10)
      else
          @papers = Paper.order("#{orderParam}  #{order}").paginate(:page => params[:page], :per_page => 10)
      end
    else
        @papers = Paper.where(:id => session[:filter_papers_id]).paginate(:page => params[:page], :per_page => 10)
    end
  ...
  def assign_fake_attribute(papers)
    papers.each{
      ...
      #count answer time
      answer_time = 0
      answer_logs = StudentAnswerLog.where(:question_id => paper.questions.pluck(:id)).order(:created_at)
  ...
  def filter
    ...
    platform_type = params[:filter][:platform_type]    
    ...
    if platform_type.present?
      @filter_papers = @filter_papers.where(:platform_type => platform_type)
    end
    if visible.present?
